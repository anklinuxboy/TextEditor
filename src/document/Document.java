...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		String[] tokens = word.toLowerCase().split("[bcdfghjklmnpqrstvwxz]+");
		List<String> list = new ArrayList<String>(Arrays.asList(tokens));
		list.removeAll(Arrays.asList("", null));
		int syllables = list.size();
		int count = 0;
		for (String str : list) {
			//System.out.println("tokenstr: " + str);
			count++;
			if (word.charAt((word.length()-1)) == 'e' && count > 1 && count == list.size() && str.equals("e")) {
				//System.out.println("Subtracting last e");
				syllables--;
			}
		}
	...
	public double getFleschScore()
	{
		...
		if (words != 0 && sentences != 0) {
			score = 206.835 - 1.015*((double) words/sentences) - 84.6*((double)syllables/words);
		}
	    return score;
	}
